...
const Dashboard = () => {
  ...
  useEffect(() => {
    async function getProjectsCount() {
      try {
        const response = await axios.get('http://localhost:8000/api/projects/count');
    ...
    async function getTasksWithProjectInfo() {
      try {
        const response = await axios.get('http://localhost:8000/api/tasks/TasksWithProjectInfo');
